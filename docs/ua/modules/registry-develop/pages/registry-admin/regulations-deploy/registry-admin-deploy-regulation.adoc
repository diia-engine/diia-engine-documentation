...
* Рекомендуємо встановити плагін https://plugins.jetbrains.com/plugin/7272-gerrit[Gerrit] для IntelliJ IDEA.
...
TIP: Повний опис xref:arch:archive/registry-regulations-deployment.adoc[моделі розгортання регламенту та пайплайн публікацій].

NOTE: У разі виникнення помилок на етапі збірки регламенту реєстру, необхідно провести дебаг для їх виявлення та виправлення.
Детальніше про проведення дебагу читайте на сторінці xref:registry-admin/debug-registry-regulations.adoc#jenkins-debug[Дебаг під час моделювання регламенту реєстру].