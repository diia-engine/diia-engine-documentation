...
TIP: Детальний список рекомендованих інструментів для розробників та адміністраторів реєстру ви можете переглянути на сторінці xref:registry-develop:study-project/index.html#preconditions-setup[Навчальний курс для розробників цифрового регламенту реєстру: що необхідно для початку роботи].
