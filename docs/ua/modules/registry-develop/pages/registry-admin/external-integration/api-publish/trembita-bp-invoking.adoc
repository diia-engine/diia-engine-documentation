...
=== Отримання WSDL-файлу для bp-webservice-gateway

. Перейдіть до консолі OpenShift (OKD) та знайдіть pod `*bp-webservice-gateway*`.
. Відкрийте термінал відповідного pod’а.
. Виконайте команду для отримання WSDL-файлу.

[NOTE]
Оскільки опис схем може містити досить великий обсяг інформації, виконується стискання *`gzip`* з подальшим перетворенням результату у рядок формату *`Base64`*.
----
wget -qO - http://localhost:8080/ws/bpWebservice.wsdl | gzip | base64 | tr -d '\n'
----

=== Декодування WSDL-файлу

. У локальному терміналі виконайте команду для декодування та розпакування WSDL-файлу:
----
echo "[BASE64_STRING]" | base64 -d | gunzip > service.wsdl
----
*Для Windows (PowerShell)*

У PowerShell виконайте наступні команди:
----
$str = "[BASE64_STRING]"
$bytes = [System.Convert]::FromBase64String($str)
[System.IO.File]::WriteAllBytes("service.wsdl.gz", $bytes)
----
Після цього розпакуйте файл service.wsdl.gz будь-яким архіватором, що підтримує формат gzip (наприклад, 7-Zip).
