...
Видалення відбувається шляхом додавання нового `changeSet` для видалення конкретної таблиці та її історичної таблиці (назва історичної таблиці така ж, як в основної, але має суфікс `_hst`. *Приклад*: таблиця `table-name` матиме історичну таблицю `table-name_hst`).
