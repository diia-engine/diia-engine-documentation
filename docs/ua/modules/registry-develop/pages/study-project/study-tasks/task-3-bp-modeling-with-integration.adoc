...
2. xref:registry-develop:bp-modeling/bp/modeling-options.adoc#camunda-modeler[Моделювання у десктопному додатку Camunda Modeler] та подальшим додавання файлу бізнес-процесу до віддаленого репозиторію Gerrit (або з перенесенням коду у веб-версію).
====
...
. Якщо ви використовуєте додаток *Camunda Modeler*, створіть нову діаграму BPMN. Для цього у лівому верхньому куті натисніть меню *File* -> *New File* -> *BPMN Diagram*:
+
...
[TIP]
Необхідно додавати подію xref:registry-develop:bp-modeling/bp/bpmn/events/timer-event.adoc[Timer] до кожної користувацької задачі для того, щоб незавершені з будь-яких причин бізнес-процеси не накопичувались у системі, а xref:registry-develop:registry-audit-instruction/modules/bp-audit.adoc#_bp_15[автоматично закривалися].
...
[TIP]
====
У цьому прикладі назва ресурсу це назва Search Condition.
Поле Variable Assignment Value використовується для визначення назви ресурсу (ендпоінт), завдяки якому буде йти пошук необхідних даних у БД.

Зверніть увагу, що при моделюванні бізнес-процесу необхідно використовувати назву Search Condition через дефіс "-"(замість нижнього підкреслювання “_”, як у *.xml файлі в якому описано Search Condition), що складаються з 2-х і більше слів.

Наприклад:

* Назва ресурсу у бізнес-процесі:  laboratory-equal-edrpou-name-count
* Назва ресурсу у *.xml файлі в якому він описаний:  laboratory_equal_edrpou_name_count
====
...
[TIP]
Змінна *response* це відповідь з фабрики даних отримана на кроці _Створення сервісної задачі для пошуку даних про лабораторію_.
...
Назва ресурсу у *.xml файлі в якому він описаний: `laboratory_test`
====
* у полі `Payload` введіть `${dataPayload}` дані для збереження;
[TIP]
Змінна *dataPayload* це змінна створена та збережена у скрипті на кроці _Створення задачі скриптування "Підготовка даних до запису (transient var)"_.
...
Додайте *Intermediate/Boundary event* з панелі інструментів  до Сервісної задачі *”Зберегти дані до Фабрики даних”*. Натисніть на знак ключа і виберіть *“Error boundary event”*.

Створіть *Gateway* перед подією *“Підписати дані про лабораторію”* - у разі виникнення помилки під час збереження даних до дата фабрики, користувач буде перенаправлений до події “Підписати дані про лабораторію”.
...
TIP: В результаті маємо змодельований складний бізнес-процес із  інтеграціями.
...
Якщо ви моделювали процес у веб застосунку Кабінет адміністратора регламентів, потрібно зберегти зміни та дочекатись коли регламент опублікується.
...
[TIP]
====
В поле Property name треба вносити ім’я поля таблиці в яке воно потрапляє у разі зберігання значення в базі. Якщо ім’я поля складається з 2 та більше слів то потрібно застосовувати CamelCase (перше слово починається з маленької літери а наступні слова починаються з великою літери  без пробілів між ними).
...
[IMPORTANT]
====
Аналогічно змоделюйте текстові поля (*Text Field*) для `Код ЄДРПОУ або РНОКПП`, `Адреса`.

//Поле `Код ЄДРПОУ або РНОКПП` повинен бути обов'язковим та мати `Property name` = `edrpou`.
====
[TIP]
====
В даному випадку адреса - звичайне текстове поле. Для використання адреси рекомендоване використання довідників.
====
[NOTE]
====
На етапі створення UI форми важливо встановлювати валідаційні правила для перевірки даних, які вносить користувач. Валідація має бути встановлена на всіх полях, які отримувач послуг має вносити самостійно.
====
...
* `/officer` -- вказує, що запит до довідника буде виконано із Кабінету посадової особи. Такий вказівник встановлюється лише для посадової особи, у випадку з отримувачем послуг citizen, нічого додатково перед /api вказувати не треба;
* `/api/data-factory/` -- вказує шлях до фабрики даних;
...
Під час створення реєстру, мають бути реалізовані додаткові можливості. Наприклад: налаштування автоматичного закриття незавершеного  користувачем бізнес-процесу, можливість повернутися на попередній етап або відразу до головного меню, налаштування сповіщень користувачу у його персональний кабінет тощо.


Основні вимоги та приклади реалізацій описані у розділі xref:registry-develop:study-project/requirements-and-recommendations.adoc[Вимоги та рекомендації розробнику щодо моделювання регламенту реєстра].
