...
*Налаштування події обробки помилок*

Для обробки помилок, які можуть виникнути на етапі збереження даних в Дата фабрику, необхідно додати та налаштувати подію обробки помилок. Для цього з бокового меню необхідно додати на сервісну задача “Зберегти дані в Дата фабрику” Intermediate/Boundary event (1), в налаштуваннях обрати тип події - Error boundary event (2) і виконати наступні налаштування (3):

* в полі Global error reference обрати “Create new”
* в полі Name – назва помилки, яку буде відловлювати делегат, Exception
* в полі Code – java.lang.Exception

image:registry-develop:study-project/task-3/t5-2.png[]

[NOTE]
====
* В змінну Message variable (errorMsg) буде зберігатись текст помилки. Зверніть увагу, що текст помилки буде повертатись тільки якщо реєстр оновлено до версії *1.9.8.68.15*. Якщо версія вашого реєстра нижча треба його оновити.
* В змінну Code variable (errorCode) буде зберігатись код помилки. Зверніть увагу, що більшість помилок не належать до типу BpmnError, тому змінна Code variable у таких випадках залишатиметься порожньою.
====
Детальніше про налаштування xref:registry-develop:bp-modeling/bp/bpmn/subprocesses/transaction-subprocess/error-event-transaction.adoc[Події обробки помилок].
