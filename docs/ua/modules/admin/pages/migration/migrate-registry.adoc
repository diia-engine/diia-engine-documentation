...
----
export SRC_CLUSTER_LOGIN="oc login --token=<TOKEN> --server=<SERVER>"
----
+
Вставте замість <TOKEN> та <SERVER> значення, отримані в пункті 1 команди логіну для кластера SRC. В кінці логін-команди не повинно бути перенесення на наступний рядок.


+
.Експорт логіну для кластера В
----
export DEST_CLUSTER_LOGIN="oc login --token=<TOKEN> --server=<SERVER>"
----
+
Вставте замість <TOKEN> та <SERVER> значення, отримані в пункті 1 команди логіну для кластера В. В кінці логін-команди не повинно бути перенесення на наступний рядок.
